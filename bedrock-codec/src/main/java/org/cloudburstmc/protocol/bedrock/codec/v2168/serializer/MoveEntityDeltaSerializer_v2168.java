...
public class MoveEntityDeltaSerializer_v2168 extends MoveEntityDeltaSerializer_v419 {
    ...
    public void deserialize(ByteBuf buffer, BedrockCodecHelper helper, MoveEntityDeltaPacket packet) {
        ...
        packet.setX(helper.readOptional(buffer, 0f, b -> { packet.getFlags().add(HAS_X); return b.readFloatLE(); }));
        packet.setY(helper.readOptional(buffer, 0f, b -> { packet.getFlags().add(HAS_Y); return b.readFloatLE(); }));
        packet.setZ(helper.readOptional(buffer, 0f, b -> { packet.getFlags().add(HAS_Z); return b.readFloatLE(); }));
        packet.setPitch(helper.readOptional(buffer, 0f, b -> { packet.getFlags().add(HAS_PITCH); return helper.readByteAngle(b); }));
        packet.setYaw(helper.readOptional(buffer, 0f, b -> { packet.getFlags().add(HAS_YAW); return helper.readByteAngle(b); }));
        packet.setHeadYaw(helper.readOptional(buffer, 0f, b -> { packet.getFlags().add(HAS_HEAD_YAW); return helper.readByteAngle(b); }));
