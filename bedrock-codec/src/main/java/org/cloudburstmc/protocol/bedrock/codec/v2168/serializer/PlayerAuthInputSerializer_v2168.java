...
public class PlayerAuthInputSerializer_v2168 extends PlayerAuthInputSerializer_v944 {
    ...
    public void serialize(ByteBuf buffer, BedrockCodecHelper helper, PlayerAuthInputPacket packet) {
        ...
        for (PlayerAuthInputData flag : packet.getInputData()) VarInts.writeInt(buffer, flag.ordinal());
    ...
    public void deserialize(ByteBuf buffer, BedrockCodecHelper helper, PlayerAuthInputPacket packet) {
        ...
        if (buffer.readBoolean()) {
            ...
            for (int i = 0; i < count; i++) packet.getInputData().add(PlayerAuthInputData.values()[VarInts.readInt(buffer)]);
        }
        ...
        if (buffer.readBoolean() && buffer.readBoolean()) packet.setItemUseTransaction(this.readItemUseTransaction(buffer, helper));
        if (buffer.readBoolean() && buffer.readBoolean()) packet.setItemStackRequest(helper.readItemStackRequest(buffer));
        if (buffer.readBoolean() && buffer.readBoolean()) helper.readArray(buffer, packet.getPlayerActions(), this::readPlayerBlockActionData, 100);
        if (buffer.readBoolean() && buffer.readBoolean()) packet.setVehicleRotation(helper.readVector2f(buffer));
        if (buffer.readBoolean() && buffer.readBoolean()) packet.setPredictedVehicle(VarInts.readLong(buffer));
    ...
    protected ItemUseTransaction readItemUseTransaction(ByteBuf buffer, BedrockCodecHelper helper) {
        ...
        if (buffer.readBoolean() && buffer.readBoolean()) helper.readInventoryActions(buffer, transaction.getActions());
